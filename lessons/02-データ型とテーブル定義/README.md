...
- collation名の `0900` は Unicode Collation Algorithm (UCA) のバージョン 9.0.0 を指す。
  `utf8mb4_general_ci` はUCAに準拠しない簡易的な比較ロジックで、絵文字のような
  基本多言語面の外側の文字を厳密に処理できない(比較・ソートが不正確になりうる)。
  これは charset(保存できる文字種)とは別レイヤーの話で、`general_ci` でも
  4byte文字は普通に**保存はできる**(保存の可否と比較の正確さを混同しないこと)。
...
- `utf8mb4_general_ci` が非推奨な理由を「対応していない文字が保存できず事故る」と誤解した。
  実際は charset(utf8mb4)の時点で4byte文字は保存できており、`general_ci`の問題は
  保存の可否ではなく**比較・ソートの正確さ**。charset(保存)とcollation(比較)の
  レイヤー分離を、この期に及んでまだ混同していた。
